...
class AirQualityFetcher:
    def __init__(self, config_path: Path | None = None) -> None:
        ...
        self.logger = get_logger("fetch_air_quality")

    def _load_source_config(self) -> AirQualitySourceConfig:
        with self.config_path.open("r", encoding="utf-8") as file:
            config = yaml.safe_load(file)

        air_quality_cfg = config["sources"]["air_quality"]
        raw_directory = self.repo_root / air_quality_cfg["raw_directory"]

        return AirQualitySourceConfig(
            source_name=air_quality_cfg["source_name"],
            base_url=air_quality_cfg["base_url"],
            raw_directory=raw_directory,
            field_parameter=air_quality_cfg["field_parameter"],
            fields_to_request=air_quality_cfg["fields_to_request"],
            recommended_parameters=air_quality_cfg.get("recommended_parameters", {}),
        )

    def _build_request_params(self, request_data: AirQualityRequest) -> dict[str, Any]:
        api_fields = [
            field
    ...
    def fetch_air_quality(self, request_data: AirQualityRequest) -> dict[str, Any]:
    ...
    def _build_file_stem(self, request_data: AirQualityRequest) -> str:
    ...
    def save_raw_payload(
        ...
        request_data: AirQualityRequest,
    ) -> Path:
        ...
        output_path = output_directory / f"air_quality_{timestamp}.json"
    ...
    def run(
        ...
        try:
            ...
            payload = self.fetch_air_quality(request_data)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch Open-Meteo air quality data for a station coordinate.",
    )
...
def main() -> None:
    ...
    print(f"Saved raw air quality JSON to: {raw_path}")
    print()
